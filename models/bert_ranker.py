...
class BertRanker(nn.Module):
    ...
    def get_research_score(self, resume_text, jd_text, mandatory_skills=None, experience_gap=0, alpha=0.7, beta=0.2, gamma=0.1):
        """
        ...
        if mandatory_skills:
            ...
            if skills_list:
                missing_count = sum(1 for s in skills_list if s not in resume_text.lower())
                penalty_missing = missing_count / len(skills_list)
        ...
        return max(0.0, final_score), {"sim": sim_score, "pen_miss": penalty_missing, "pen_exp": penalty_exp}
